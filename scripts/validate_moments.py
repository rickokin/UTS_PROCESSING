...
def main():
    ap = argparse.ArgumentParser(description="Validate Moment Objects against moment.schema.v1.json")
    ap.add_argument("input", help="Path to a JSON file (single object or array) OR a folder of .json files")
    ap.add_argument("--schema", default=str(Path(__file__).resolve().parent.parent / "schemas" / "moment.schema.v1.json"))
    args = ap.parse_args()
    ...
    targets = []
    if in_path.is_dir():
        targets = list(in_path.rglob("*.json"))
    else:
        targets = [in_path]
