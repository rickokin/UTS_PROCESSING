import { loadPrompts, loadSchemas, loadAllVocabText, loadExternalResearch } from '@/lib/assets/loader';
...
export async function POST(req: Request) {
  try {
    ...
    let validationRulesText = '';
    try {
      const { loadText } = await import('@/lib/assets/loader');
      validationRulesText = await loadText('rules/validation_rules.v1.yaml');
    } catch {
      validationRulesText = '';
    }
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // Save validation PDF + DOCX to reports directory
          try {
            ...
            const pdfFilePath = path.join(reportsDir, `validation_report_${timestamp}.pdf`);
            await generateValidationPDF(result.validation, pdfFilePath);

            const docxDoc = buildValidationDocx(result.validation);
            const docxBuffer = await Packer.toBuffer(docxDoc);
            const docxFilePath = path.join(reportsDir, `validation_report_${timestamp}.docx`);
