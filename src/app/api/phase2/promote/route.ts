...
`.trim();
    ...
    // Create a map of moments by ID for quick lookup
    const momentMap = new Map();
    if (moments && Array.isArray(moments)) {
      moments.forEach(m => {
        // Handle both raw moment_id and combined "episode_id/moment_id"
        momentMap.set(m.moment_id, m);
        if (m.episode_id) {
          momentMap.set(`${m.episode_id}/${m.moment_id}`, m);
          // Also store without .docx just in case Gemini stripped it during clustering
    ...
    // Enrich clusters with moment text so Gemini has the source material for exact quotes
    const enrichedClusters = clusters.map(cluster => {
      const enrichedMoments = (cluster.member_moment_ids || []).map((id: string) => {
        // Try looking up with the ID, or fallback to parsing it if it has a slash
        let m = momentMap.get(id);
        if (!m && id.includes('/')) {
          // Sometimes ID might have a missing .docx, but our map handles it now
          // If still not found, try just the M00X part
