...
import { loadDemographicsFromOutput, computeDemographicsSummary } from "@/lib/shared/loader";
...
async function loadPipelineValidation(outputDir: string, pipelineId: string): Promise<any | null> {
  const dir = pipelineOutputDir(outputDir, pipelineId);
  const candidates = [path.join(dir, "validation.json"), path.join(outputDir, "validation.json")];
...
export async function POST(req: Request, ctx: RouteContext) {
  ...
  if (pipelineId !== "insights") {
    return notImplementedResponse(pipelineId, "assemble");
  }
  ...
  try {
    ...
    // Compute actual distinct episodes/moments from the payload so the report's
    // methodology stats cannot be hallucinated by the model.
      ...
      if (Array.isArray(insight.supporting_moments)) {
        insight.supporting_moments.forEach((m: any) => {
          ...
          if (m.moment_id) {
            ...
            // combine them to get the true count of distinct moments promoted.
    ...
    // Demographics live at the shared outputDir root — they apply to every pipeline.
...
`;
    ...
    try {
      validationData = outputDir ? await loadPipelineValidation(outputDir, pipelineId) : null;
      if (validationData) {
      ...
      async start(controller) {
        ...
        try {
          ...
          if (report) {
            ...
            const docxFilePath = path.join(reportsDir, `${pipelineId}_${safeTitle}_${timestamp}.docx`);
            const pdfFilePath = path.join(reportsDir, `${pipelineId}_${safeTitle}_${timestamp}.pdf`);
